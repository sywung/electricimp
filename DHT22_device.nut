...
class DHT22 {
    ...
    static ONE               = 0.000076;  // 70 us high for "1"
    ...
    function parse(hexblob) {
        ...
        for (byte = 0; byte < hexblob.len(); byte++) {
            for (bit = 7; bit >= 0; bit--) {
                
                thisbit = (hexblob[byte] & (0x01 << bit)) ? 1:0;
                //t1=t1+thisbit;
                //if (thisbit != laststate) server.log("change");
                if (thisbit != laststate) {
                    server.log(format("change %d",byte));
                    if (thisbit) {
                        // low-to-high transition; watch to see how long it is high
                        laststate = 1 ;
                        lastbitidx = (8 * byte) + (7 - bit);
                        server.log(laststate);
                    } else {
